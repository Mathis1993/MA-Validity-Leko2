...
$\mathbf{CISRT} = correct * (1 - \frac{latency}{bound_{upper}}) * 100)$ 
...
#8. Guessing Correction

**???**

```{r guessing correction, warning = FALSE, message = FALSE}
#elfe_quop$w_cisrt_t1 <- ifelse(elfe_quop$w_rcp_t1<=50, NA, elfe_quop$w_cisrt_t1)
#elfe_quop$s_cisrt_t1 <- ifelse(elfe_quop$s_rcp_t1<=50, NA, elfe_quop$s_cisrt_t1)
#elfe_quop$t_cisrt_t1 <- ifelse(elfe_quop$t_rcp_t1<=50, NA, elfe_quop$t_cisrt_t1)
```

#9. Reliability of the Quop Tests
...
###10.1.2 Discrimant Status Validity
...
####10.1.2.1 Attenuated Discriminant Status Validity
...
###10.1.3 Convergent Status Validity
...
####10.1.3.1 Attenuated Convergent Status Validity
...
###10.1.4 Criterial Status Validity
...
##10.2 Validity of Change Measurement
...
Extract datasets for each scale and rename the columns to be a bit more handy. (Also, only keep cases for which both ELFE tests (pre and post) have been completed. --> Not done right now, why lose information?)
...
ch_val_model <- '
                ...
                #Mean structure: Restrict intercepts of both first parcels to 0, freely 
                #estimate the means of ELFE_pre and the difference
                #e1_p1 ~ 0*1
                #e2_p1 ~ 0*1
                #ELFE_pre ~ 1
                #D_ELFE ~ 1
      
                #Mean structure: Restrict the intercepts of the other parcels to the same value
                #over measurements, too (although not necessarily to 0)
                #e1_p1 ~ f*1
                #e2_p1 ~ f*1
                #e1_p2 ~ c*1
                #e2_p2 ~ c*1
                ...
                #Mean Structure
                #We are only interested in the means of the LV, so restrict the first two indicators intercepts to 0
                #to be able to estimate the LV means
                #q1 ~ 0*1
                #q2 ~ 0*1
                #intercept ~ 1
                #slope ~ 1
...
Indicator 1: Influence of intercept and slope of a linear growth model estimated over all quop scores (testing points 1 to 8) on elfe post scores, incremental to explaining the elfe post scores with the elfe pre scores.

**ADD PICTURE OF MODEL**

Indicator 2: Influence of intercept and slope of a linear growth model estimated over quop scores (testing points 2 to 7) on quop testing point 8 scores, incremental to explaining the quop testing point 8 scores with the quop testing point 1 scores.
...
**LIKE IN MPLUS**: Intercepts of quop indicators are restricted to 0 and means for slope and intercept are estimated. Why?

Also: For sentence scale, weird results without providing starting values for the regression from MPLUS.

Without the MPLUS-default restrictions of the quop indicators and estimating the means (so without doing anything about the mean structure), the model does not converge on word level, but the result pattern on sentence and text level is the same. Also in MPLUS, no convergence on word scale in this case.

**WITHOUT missing="fiml"**: All models work (but for change validity withput fiml: no longer significance on texd level)

**ToDo**: 

1. Why does MPLUS have these defaults? Models with or without them?
2. If without them, why no convergence on word level?
3. If with them, why weird results on sentence scale without providing starting values?

**UPDATE**: When only using cases for which both elfe tests have been completed, using the MPLUS defaults or not makes no difference anymore. But, now in both cases, there are weird values on sentence level. In MPLUS (with its defaults), there are no weird results on sentence level.
Again, in this case, without missing="fiml", no signifiance of change validity model on text level anymore, but no weird results for the predictive validity on text level, either.

**UPDATED ToDo**:
1. Get lavaan to estimate proper results on sentence level, like MPLUS does (without using any mean structure).

**UPDATE 061119**: With changed CISRT (also use lower bound, word and sentence scale make problems. With the starting values below, everything is fine (no mean structure)).
...
pr_val_ind1_model <- '
                      ...
                      #Starting values for word scale 
                      #ELFE_post ~ start(0.6)*ELFE_pre + start(0.88)*inter + start(0.14)*slope

                      #Starting values for sentence scale
                      #ELFE_post ~ start(0.7)*ELFE_pre + start(0.8)*inter + start(0.07)*slope
                     '
```
...
AS we only use quop scores here, we can estimate the models based on the whole sample and not just the subsample having completed the standardized tests.

Extract datasets for each scale and rename the columns to be a bit more handy.

```{r dataset per scale predictive, warning = FALSE, message = FALSE}
##Extract
#Word
pr_val_wort <- quop_use[c("code","class", colnames(quop_use)[grep("(^w_)cisrt", colnames(quop_use))], colnames(quop_use)[grep("(^w_)(t1|t8)_parcel", colnames(quop_use))])]

#Sentence
pr_val_satz <- quop_use[c("code","class", colnames(quop_use)[grep("(^s_)cisrt", colnames(quop_use))], colnames(quop_use)[grep("(^s_)(t1|t8)_parcel", colnames(quop_use))])]

#Text
pr_val_text <- quop_use[c("code","class", colnames(quop_use)[grep("(^t_)cisrt", colnames(quop_use))], colnames(quop_use)[grep("(^t_)(t1|t8)_parcel", colnames(quop_use))])]

##Rename (MPlus only displays variable names up to 8 characters long)
#Word
names(pr_val_wort) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)))

#Sentence
names(pr_val_satz) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)))

#Text
names(pr_val_text) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)))
```

Implement Predictive Validity Model: We are interested in the influence of the latent intercept and slope estimated over quop scores (testing points 2 to 7) on quop testing point 8 scores incremental to quop testing point 1 scores.

```{r predictive validity model indicator 2, warning = FALSE, message = FALSE}
pr_val_ind2_model <- '
                      quop1 =~ q1_p1 + q1_p2 + q1_p3
                      quop8 =~ q8_p1 + q8_p2 + q8_p3

                      intercept =~ 1*q2 + 1*q3 + 1*q4
                                 + 1*q5 + 1*q6 + 1*q7

                      slope =~ 0*q2 + 1*q3 + 2*q4
                             + 3*q5 + 4*q6 + 5*q7

                      quop8 ~ quop1 + intercept + slope
                     '
```
...
```{r estimate word model predicitve ind2, warning = FALSE, message = FALSE}
pr_val_ind2_wort.fit <- cfa(pr_val_ind2_model, data=pr_val_wort, estimator="mlr")
#summary(pr_val_ind2_wort.fit, std=T, fit.measures=T)
```

**Sentence**

```{r estimate sentence model predictive ind2, warning = FALSE, message = FALSE}
pr_val_ind2_satz.fit <- cfa(pr_val_ind2_model, data=pr_val_satz, estimator="mlr")
#summary(pr_val_ind2_satz.fit, std=T, fit.measures=T)
```

**Text**

```{r estimate text model predictive ind2, warning = FALSE, message = FALSE}
pr_val_ind2_text.fit <- cfa(pr_val_ind2_model, data=pr_val_text, estimator="mlr")
#summary(pr_val_ind2_text.fit, std=T, fit.measures=T)
```

#11. MPLUS Datasets 

At the moment, the models are estimated in MPLUS. Here, the data is prepared accodingly.

From the subsample dataframe, build an MPLUS dataset containing all variables needed in the correct format (Validity of Change Measurement and Predictive Validity Indicator 1).

Relevant Variables are elfe parcels, quop scores and quop parcels t1&t8 (elfe subsample).

Format requirements: 

* No variable names in the dataset to be read into MPLUS

* When naming the variables inside the MPLUS script, they may only be up to 8 characters long

```{r elfe subsample for mplus, warning = FALSE, message = FALSE}
##Extract
#word
#elfe_quop_wort_mplus <- elfe_quop[c("code","class", colnames(elfe_quop)[grep("(^w_)cisrt", colnames(elfe_quop))], colnames(elfe_quop)[grep("(^w_)(t1|t8)_parcel", colnames(elfe_quop))], colnames(elfe_quop)[grep("(^w_)elfe", colnames(elfe_quop))])]

#sentence
#elfe_quop_satz_mplus <- elfe_quop[c("code","class", colnames(elfe_quop)[grep("(^s_)cisrt", colnames(elfe_quop))], colnames(elfe_quop)[grep("(^s_)(t1|t8)_parcel", colnames(elfe_quop))], colnames(elfe_quop)[grep("(^s_)elfe", colnames(elfe_quop))])]

#text
#elfe_quop_text_mplus <- elfe_quop[c("code","class", colnames(elfe_quop)[grep("(^t_)cisrt", colnames(elfe_quop))], colnames(elfe_quop)[grep("(^t_)(t1|t8)_parcel", colnames(elfe_quop))], colnames(elfe_quop)[grep("(^t_)elfe", colnames(elfe_quop))])]

##Rename (MPlus only displays variable names up to 8 characters long)
#Word
#names(elfe_quop_wort_mplus) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)), paste0("e",rep(1:2,each=3),"_p",rep(1:3,times=2)))

#Sentence
#names(elfe_quop_satz_mplus) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)), paste0("e",rep(1:2,each=3),"_p",rep(1:3,times=2)))

#Text
#names(elfe_quop_text_mplus) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)), paste0("e",rep(1:2,each=3),"_p",rep(1:3,times=2)))

elfe_quop_wort_mplus <- ch_val_wort
elfe_quop_satz_mplus <- ch_val_satz
elfe_quop_text_mplus <- ch_val_text

##Export
#Word
elfe_quop_wort_mplus[is.na(elfe_quop_wort_mplus)] <- -99
write.table(elfe_quop_wort_mplus,"elfe_quop_wort_mplus.dat",sep=",",row.names=FALSE, col.names=FALSE)
write.table(elfe_quop_wort_mplus,"elfe_quop_wort_mplus_var_names.txt",sep=",") #export with variable names

#Sentence
elfe_quop_satz_mplus[is.na(elfe_quop_satz_mplus)] <- -99
write.table(elfe_quop_satz_mplus,"elfe_quop_satz_mplus.dat",sep=",",row.names=FALSE, col.names=FALSE)
write.table(elfe_quop_satz_mplus,"elfe_quop_satz_mplus_var_names.txt",sep=",") #export with variable names

#Text
elfe_quop_text_mplus[is.na(elfe_quop_text_mplus)] <- -99
write.table(elfe_quop_text_mplus,"elfe_quop_text_mplus.dat",sep=",",row.names=FALSE, col.names=FALSE)
write.table(elfe_quop_text_mplus,"elfe_quop_text_mplus_var_names.txt",sep=",") #export with variable names

```

Build an MPLUS dataset from all quop data containing all variables needed in the correct format (Predictive Validity Indicator 2).

Relevant Variables are quop scores and quop parcels t1&t8 (all subjects).

```{r quop wholesample mplus, warning = FALSE, message = FALSE}
##Extract
#Word
quop_wort_mplus <- quop_use[c("code","class", colnames(quop_use)[grep("(^w_)cisrt", colnames(quop_use))], colnames(quop_use)[grep("(^w_)(t1|t8)_parcel", colnames(quop_use))])]

#Sentence
quop_satz_mplus <- quop_use[c("code","class", colnames(quop_use)[grep("(^s_)cisrt", colnames(quop_use))], colnames(quop_use)[grep("(^s_)(t1|t8)_parcel", colnames(quop_use))])]

#Text
quop_text_mplus <- quop_use[c("code","class", colnames(quop_use)[grep("(^t_)cisrt", colnames(quop_use))], colnames(quop_use)[grep("(^t_)(t1|t8)_parcel", colnames(quop_use))])]

##Rename (MPlus only displays variable names up to 8 characters long)
#Word
names(quop_wort_mplus) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)))

#Sentence
names(quop_satz_mplus) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)))

#Text
names(quop_text_mplus) <- c("code", "class", paste0("q",seq(1,8)), paste0("q",rep(c(1,8),each=3),"_p",rep(1:3,times=2)))

##Export
#Word
quop_wort_mplus[is.na(quop_wort_mplus)] <- -99
write.table(quop_wort_mplus,"quop_wort_mplus.dat",sep=",",row.names=FALSE, col.names=FALSE)
write.table(quop_wort_mplus,"quop_wort_mplus_var_names.txt",sep=",") #export with variable names

#Sentence
quop_satz_mplus[is.na(quop_satz_mplus)] <- -99
write.table(quop_satz_mplus,"quop_satz_mplus.dat",sep=",",row.names=FALSE, col.names=FALSE)
write.table(quop_satz_mplus,"quop_satz_mplus_var_names.txt",sep=",") #export with variable names

#Text
quop_text_mplus[is.na(quop_text_mplus)] <- -99
write.table(quop_text_mplus,"quop_text_mplus.dat",sep=",",row.names=FALSE, col.names=FALSE)
write.table(quop_text_mplus,"quop_text_mplus_var_names.txt",sep=",") #export with variable names
```

The models described in 10.2 and 10.3 are estimated in MPLUS.
