...
for(i in time_pts){
  for(j in 1:length(scales)){
    for(k in items_start[j]:items_end[j]){
      eval(parse(text=paste0("quop_use$t",i,"_cisrt",k,"<- quop_use$t",i,"_rc",k,"*(1-(quop_use$t",i,"_gc",k,"/",scales[j],"_ub))*100")))
    }
  }
}
```
...
pr_val_ind1_model <- '
                      ...
                      #Starting values for word scale when NOT using MPLUS defaults
                      #ELFE_post ~ start(0.6)*ELFE_pre + start(0.88)*inter + start(0.14)*slope

                      #Starting values for sentence scale when using MPLUS defaults
